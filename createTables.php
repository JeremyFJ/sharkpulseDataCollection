 ...
 */
    ...
    $sp_sql = "SELECT id, date, time, users_email, species_name, latitude, longitude, img_name, notes FROM sharkpulse_temp";
    ...
    if (!$mobile_result && !$sptemp_result) {
	    ...
	    echo '<tr>';
            echo "<td>". $row[0] ."</td> <td>".$row[1]."</td><td>". $row[2]."</td><td>".$row[3]. "</td><td>$row[4]</td><td>" .$row[5].
                "</td><td>".$row[6]. "</td><td>". $row[8]. '</td><td>'.$row[9].'</td><td><img src=/~edsan'.$row[7].' height="10%"></td>
